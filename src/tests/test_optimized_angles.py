...
def optimize_angles_increasing_n_nodes(run_name, start_n, end_n,*args, fun=nx.barabasi_albert_graph, iter=100, n_graphs=10, algo_name="aqaoa", p=1, step=50, **kwargs):
    ...
    for n in n_nodes_lst:
        ...
        for j in range(n_graphs):
            G = create_graph(rp=rp, fun=fun, n=n, index=f"_{n}_nodes{j}", *args, **kwargs)
            graphs.append(G)
        print("Optimization Start... 😙")
        oa = OptimizedAngles()
        for G in graphs:
            row = oa.compute(G=G, algo_name=algo_name, p=p, iter=iter)
            rows.append(row)
